...
_, drifters = extract_tracked_drifter_data(label)
...
x = 1.0 : 10.0
y = 1.0 : 10.0
hm = heatmap!(ax, xb/1kilometer, yb/1kilometer, b_xy; colorrange = (-0.5*b_max, 1.5*b_max))
#scatter!(ax, 5randn(10) .+ 30, 5randn(10) .+ 30, marker = '.', markersize = 50, color = :red)
scatter!(ax, xs/1kilometer, ys/1kilometer, marker = '.', markersize = 50, color = :red)
...
CairoMakie.record(i -> iter[] = i,
        fig,
        pp_dir(label) * "bζδ-top-vid.mp4",
        iterations[maximum([Int64(round(length(iterations) * 0.0)), 1]) : 2 : Int64(round(length(iterations) * 0.3))],
        framerate = 20)=#